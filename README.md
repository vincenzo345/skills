...
- [`claude-rigor`](./skills/claude-rigor/SKILL.md) — install and inspect the direct-GitHub Claude Code harness that applies a compact Codex-aligned operating contract, guards deployed applications during read-only investigations, and retains a one-shot review backstop for code changes.
...
- [`feature-planner`](./skills/feature-planner/SKILL.md) — turn repository evidence and product intent into an implementation-ready feature plan.
- [`openai-docs`](./skills/openai-docs/SKILL.md) — answer current OpenAI product, API, model, and Codex questions from official documentation using harness-neutral retrieval capabilities.
...
The repository includes a [24-task offline evaluation suite](./docs/claude-harness-evals.md) for testing whether a coding-agent harness produces verified repository outcomes rather than plausible-looking patches. It stages isolated agent workspaces and keeps executable acceptance programs and reference solutions on the evaluator side.
