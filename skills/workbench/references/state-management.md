...
- `next` returns a narrower view of the same frontier. Use it when that focus helps, not mechanically after `status`. For deliverable nodes, `agent_ready` excludes tickets with incomplete `depends-on` edges and projects source IDs, blockers, and proof status. Its recommendation is not a grant to perform a consequential action.
- `claim-node` atomically changes one dependency-ready, agent-owned implementation ticket from `ready` to `in-progress` using the expected state revision. A stale revision, incomplete dependency, or duplicate claim fails without mutation.
